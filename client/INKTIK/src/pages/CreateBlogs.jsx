...
const CreateBlogs = () => {
    ...
    const [isPostCreated, setIsPostCreated] = useState(false); // New state variable
  ...
  return (
    ...
    <div className='flex w-full h-full relative'>
      ...
      <div className="left fixed md:w-[20%] hidden md:block overflow-y-hidden max-h-screen">
        <img src={loginImg} className='object-cover w-full min-h-screen' alt="Signup Background"/>
      </div>
      
      {/* Right Section with Form */}
      <div className="right md:w-[80%] flex items-center flex-col justify-center w-full h-screen m-2 relative md:pl-16 ">
        <div className='flex items-start sm:justify-center sm:items-center w-full mb-14 ml-4 sm:ml-0'> {/* Flex container for left-alignment */}
          <div className='w-44'>
        ...
        <div className='w-full flex flex-col items-center justify-center'>
          <div className='flex flex-col items-start justify-center w-full max-w-lg  p-2 h-[30rem] md:ml-10'>
           
            <form onSubmit={createdBlog} method="post"  encType="multipart/form-data" className="flex flex-col mt-48 w-full">
              <h1 className="text-left text-4xl sm:text-5xl font-bold mb-10">
                  Create Blog 
                </h1>
                {" "}
...
<input
  ...
  className="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow my-2"
  placeholder="Title"
/>
                ...
                <label >Description</label>
                  ...
                  onChange={(e) => setDescription(e.target.value)}
                    className="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow my-2 resize-none "
...
<p className="pt-4 px-3 line-clamp-1 text-ellipsis">{image ? image.name : ''}</p>
                ...
                <button className="bg-[#000] text-white py-2 rounded-3xl mt-4 px-5 w-24">
                  Post
                </button>
