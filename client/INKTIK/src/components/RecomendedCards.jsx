...
const RecomendedCards = ({ _id, title, cover, description, summary }) => {
      ...
      <div className="rounded-xl  flex flex-col w-full">
        <div className="relative">
          <Link to={`/blog/blog-user-page/${_id}`}>
            <img
              className="hover:-translate-y-1 duration-500 transition-all rounded-2xl min-w-full h-[12rem] md:h-[14rem] object-cover"
              src={cover}
              alt="Blog Cover"
            />
          </Link>
  ...
  <Link to={`/blog/blog-user-page/${_id}`} className="block max-w-xs">
    <h5 className="text-gray-900 font-bold text-lg  mb-2 cursor-pointer hover:underline tracking-wider line-clamp-2 text-ellipsis break-words">
      {title}
    </h5>
  </Link>
</div>
